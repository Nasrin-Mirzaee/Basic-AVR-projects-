...
;#include <delay.h>
;
;#define seven PORTB   // 7-segment display data lines connected to PORTB
;#define d1 PORTC.0    // First 7-segment digit enable (common pin)
;#define d2 PORTC.1    // Second 7-segment digit enable
;#define on 0          // Define logic level for enabling a digit (active low)
;#define off 1         // Define logic level for disabling a digit
;#define led_8 PORTA   // 8 LEDs connected to PORTA
;#define led_1 PORTD.7 // Single LED connected to PD7
;
;unsigned char x = 1;      // Used for LED shifting on PORTA
;unsigned char i, j;       // Counters for digits (two-digit number display)
;unsigned char t_seven;    // Timer counter for updating 7-segment digits
;unsigned char t_led_8;    // Timer counter for shifting LEDs on PORTA
;unsigned char t_led_1;    // Timer counter for toggling single LED on PD7
;
;// Lookup table for displaying digits (0–9) on a 7-segment display
;
...
_0x3:
; 0000 001E 
; 0000 001F         // ---- Multiplexing two 7-segment displays ----
; 0000 0020         seven = digit[i]; // Display digit i
	MOV  R30,R4
	LDI  R31,0
	...
	LPM  R0,Z
	OUT  0x18,R0
; 0000 0021         d1 = on;          // Enable first digit
	CBI  0x15,0
; 0000 0022         d2 = off;         // Disable second digit
	SBI  0x15,1
; 0000 0023         delay_ms(5);      // Short delay for persistence of vision
	LDI  R26,LOW(5)
	LDI  R27,0
	CALL _delay_ms
; 0000 0024 
; 0000 0025         seven = digit[j]; // Display digit j
	MOV  R30,R7
	LDI  R31,0
	SUBI R30,LOW(-_digit*2)
	SBCI R31,HIGH(-_digit*2)
	LPM  R0,Z
	OUT  0x18,R0
; 0000 0026         d1 = off;         // Disable first digit
	SBI  0x15,0
; 0000 0027         d2 = on;          // Enable second digit
	CBI  0x15,1
; 0000 0028         delay_ms(5);
	...
	CALL _delay_ms
; 0000 0029 
; 0000 002A         // Increment counters for timing different tasks
; 0000 002B         t_seven++;
	INC  R6
; 0000 002C         t_led_8++;
	INC  R9
...
; 0000 002E 
; 0000 002F         ///////////////////////////////
; 0000 0030         // Update 7-segment counter
; 0000 0031         if(t_seven == 50){     // ~50 * (5+5)ms = ~500ms
	LDI  R30,LOW(50)
	CP   R30,R6
	BRNE _0xE
; 0000 0032             t_seven = 0;
	CLR  R6
; 0000 0033             i++;               // Increase units digit
	INC  R4
; 0000 0034             if(i > 9){
	LDI  R30,LOW(9)
	CP   R30,R4
...
; 0000 0035                 i = 0;
	CLR  R4
; 0000 0036                 j++;           // Increase tens digit
	INC  R7
; 0000 0037                 if(j > 9){
	CP   R30,R7
	BRSH _0x10
; 0000 0038                     j = 0;     // Reset after 99 ? back to 00
	CLR  R7
; 0000 0039                     i = 0;
	CLR  R4
; 0000 003A                 }
...
; 0000 003E 
; 0000 003F         ///////////////////////////////
; 0000 0040         // Shift LEDs on PORTA
; 0000 0041         if(t_led_8 == 3){       // Every few cycles
_0xE:
	LDI  R30,LOW(3)
	CP   R30,R9
	BRNE _0x11
; 0000 0042             t_led_8 = 0;
	CLR  R9
; 0000 0043             led_8 = x;          // Send pattern to LEDs
	OUT  0x1B,R5
; 0000 0044             x <<= 1;            // Shift left
	LSL  R5
; 0000 0045             x = x == 0 ? 1 : x; // Reset to 1 if overflow
	TST  R5
	BRNE _0x12
	LDI  R30,LOW(1)
	RJMP _0x13
...
; 0000 0048 
; 0000 0049         ///////////////////////////////
; 0000 004A         // Toggle single LED on PD7
; 0000 004B         if(t_led_1 == 20){      // After some cycles
_0x11:
	LDI  R30,LOW(20)
	CP   R30,R8
	BRNE _0x15
; 0000 004C             t_led_1 = 0;
	CLR  R8
; 0000 004D             led_1 = ~led_1;     // Toggle LED (ON ? OFF)
	SBIS 0x12,7
	RJMP _0x16
	CBI  0x12,7
	RJMP _0x17
...
; 0000 0060 DDRA=(1<<DDA7) | (1<<DDA6) | (1<<DDA5) | (1<<DDA4) | (1<<DDA3) | (1<<DDA2) | (1<<DDA1) | (1<<DDA0);
	LDI  R30,LOW(255)
	OUT  0x1A,R30
...
; 0000 0065 // Function: Bit7=Out Bit6=Out Bit5=Out Bit4=Out Bit3=Out Bit2=Out Bit1=Out Bit0=Out
; 0000 0066 DDRB=(1<<DDB7) | (1<<DDB6) | (1<<DDB5) | (1<<DDB4) | (1<<DDB3) | (1<<DDB2) | (1<<DDB1) | (1<<DDB0);
	LDI  R30,LOW(255)
	OUT  0x17,R30
; 0000 0067 // State: Bit7=0 Bit6=0 Bit5=0 Bit4=0 Bit3=0 Bit2=0 Bit1=0 Bit0=0
; 0000 0068 PORTB=(0<<PORTB7) | (0<<PORTB6) | (0<<PORTB5) | (0<<PORTB4) | (0<<PORTB3) | (0<<PORTB2) | (0<<PORTB1) | (0<<PORTB0);
	LDI  R30,LOW(0)
	OUT  0x18,R30
; 0000 0069 
; 0000 006A // Port C initialization
; 0000 006B // Function: Bit7=In Bit6=In Bit5=In Bit4=In Bit3=In Bit2=In Bit1=Out Bit0=Out
; 0000 006C DDRC=(0<<DDC7) | (0<<DDC6) | (0<<DDC5) | (0<<DDC4) | (0<<DDC3) | (0<<DDC2) | (1<<DDC1) | (1<<DDC0);
	LDI  R30,LOW(3)
	OUT  0x14,R30
; 0000 006D // State: Bit7=T Bit6=T Bit5=T Bit4=T Bit3=T Bit2=T Bit1=0 Bit0=0
; 0000 006E PORTC=(0<<PORTC7) | (0<<PORTC6) | (0<<PORTC5) | (0<<PORTC4) | (0<<PORTC3) | (0<<PORTC2) | (0<<PORTC1) | (0<<PORTC0);
	LDI  R30,LOW(0)
	OUT  0x15,R30
; 0000 006F 
; 0000 0070 // Port D initialization
; 0000 0071 // Function: Bit7=Out Bit6=In Bit5=In Bit4=In Bit3=In Bit2=In Bit1=In Bit0=In
; 0000 0072 DDRD=(1<<DDD7) | (0<<DDD6) | (0<<DDD5) | (0<<DDD4) | (0<<DDD3) | (0<<DDD2) | (0<<DDD1) | (0<<DDD0);
	LDI  R30,LOW(128)
	OUT  0x11,R30
; 0000 0073 // State: Bit7=0 Bit6=T Bit5=T Bit4=T Bit3=T Bit2=T Bit1=T Bit0=T
