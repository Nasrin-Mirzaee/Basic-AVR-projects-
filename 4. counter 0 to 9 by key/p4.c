...
#define button PIND.0      //PORTD.0 AS INPUT CONNECT TO THE KEY
...
while (1)
      {
      PORTB = digit[i];     // Send the pattern for digit[i] to PORTB ? display number on 7-segment
      if(button==0){        
      delay_ms(200);
      i++;                  // Go to next digit
      i= i==10 ? 0:i;       // If i reaches 10, reset it back to 0
      }}
}
